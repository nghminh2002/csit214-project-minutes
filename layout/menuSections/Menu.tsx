...
export default function Menu({ passengerInfor }: any) {
    ...
    const handleAddMenu = (data: any, index: number, element: any) => {
        ...
        if (menuItems[index].menuSelected.length > 0) {
            if (menuItems[index].menuSelected.some((item: { img: any; }) => item.img === data.img)) {
                data.status = "default";
                menuItems[index].menuSelected.splice(menuItems[index].menuSelected.findIndex((item: { img: any; }) => item.img === data.img), 1);
            }
            else {
    ...
    const checkSelcet = (menuSelect: { [x: string]: { menuSelected: { img: any; }[]; }; },index: string | number,item: { img: any; })=>{
        return menuSelect[index].menuSelected.some((items: { img: any; }) => items.img === item.img)
    }
